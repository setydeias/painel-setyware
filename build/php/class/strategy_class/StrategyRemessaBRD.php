<?php
    ...
    class StrategyRemessaBRD implements IStrategyRemessaRegistrada {

        public function GenerateFileHeader($params) {
            $header = $params['cod_banco'].'00000'.str_pad('', 9, ' ', STR_PAD_RIGHT).$params['tipo_documento'].$params['documento'].str_pad($params['convenio'], 20, '0', STR_PAD_LEFT);
			$header .= str_pad($params['agencia'], 6, '0', STR_PAD_LEFT).str_pad($params['conta'], 13, '0', STR_PAD_LEFT);
			$header .= ' '.str_pad(substr($params['razao_social'], 0 , 30), 30, ' ', STR_PAD_RIGHT).str_pad(substr($params['banco'], 0 , 30), 30, ' ', STR_PAD_RIGHT);
			$header .= str_pad('', 10, ' ', STR_PAD_RIGHT).'1'.date('dmY').date('His').'00000108400000'.str_pad('', 69, ' ', STR_PAD_RIGHT).PHP_EOL;

			return $header;
        }

        public function GenerateLoteHeader($params) {
            $header = $params['cod_banco'].$params['lote'].'1C01'.str_pad('', 2, ' ', STR_PAD_RIGHT).$params['versao_remessa'].' '.$params['tipo_documento'].str_pad($params['documento'], 14, '0', STR_PAD_LEFT);
			$header .= str_pad($params['convenio'], 20, '0', STR_PAD_LEFT).str_pad($params['agencia'], 6, '0', STR_PAD_LEFT);
			$header .= str_pad($params['conta'], 13, '0', STR_PAD_LEFT).' '.str_pad(substr($params['razao_social'], 0 , 30), 30, ' ', STR_PAD_RIGHT).str_pad('', 40, ' ', STR_PAD_RIGHT);
			$header .= str_pad('AVENIDA LINEU MACHADO', 30, ' ', STR_PAD_RIGHT).str_pad('777', 5, ' ', STR_PAD_RIGHT).str_pad('', 15, ' ', STR_PAD_RIGHT);
            $header .= str_pad('FORTALEZA', 20, ' ', STR_PAD_RIGHT).'60520101CE'.str_pad('', 18, ' ', STR_PAD_RIGHT).PHP_EOL;
        ...
        public function SegmentoP($params, $data) {
			...
			$nosso_numero = str_pad(substr($data['nosso_numero'], 0 , 11), 11, ' ', STR_PAD_RIGHT);
			$carteira = str_pad($params['carteira'], 3, '0', STR_PAD_LEFT);
            $nosso_numero_dv = Util::Modulo11($params['carteira'].$data['nosso_numero'], 7) < 10 ? Util::Modulo11($params['carteira'].$data['nosso_numero'], 7) : 'P';
			...
			$num_documento = str_pad(substr($data['nosso_numero'], 0 , 11), 15, ' ', STR_PAD_RIGHT);
			...
			$segmento_p = $banco.$lote.$registro.$num_registro.$segmento.' '.$cod_mov.$agencia.$conta.' '.$carteira.'00000'.$nosso_numero.$nosso_numero_dv;
            $segmento_p .= $forma_cadastramento.$tipo_documento.'222'.$num_documento.$data_vencimento.$valor_titulo.str_pad('', 5, '0', STR_PAD_LEFT).' '.$especie.$aceite.$data_emissao;
			...
			$segmento_p .= $cod_protesto.$num_dias_protesto.'2'.$num_dias_baixa.$moeda.str_pad('', 10, '0', STR_PAD_LEFT).' '.PHP_EOL;
