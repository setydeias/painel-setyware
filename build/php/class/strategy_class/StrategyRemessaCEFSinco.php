<?php
    ...
    class StrategyRemessaCEFSinco implements IStrategyRemessaRegistrada {
        ...
        public function GenerateFileHeader($params) {
			...
			$header .= str_pad('', 5, '0', STR_PAD_LEFT).str_pad('', 20, ' ', STR_PAD_RIGHT).str_pad('', 20, ' ', STR_PAD_RIGHT).str_pad('', 29, ' ', STR_PAD_RIGHT).PHP_EOL;

			return $header;
        }

        public function GenerateLoteHeader($params) {
            $header = $params['cod_banco'].$params['lote'].'1R0100030 '.$params['tipo_documento'].str_pad($params['documento'], 15, '0', STR_PAD_LEFT);
        ...
        public function SegmentoP($params, $data) {
			...
			$nosso_numero = str_pad(substr($data['nosso_numero'], 0 , 18), 18, ' ', STR_PAD_RIGHT);
			$nosso_numero_dv = Util::Modulo11(trim($nosso_numero), 9) > 9 ? '0' : Util::Modulo11(trim($nosso_numero), 9);
			...
			$num_documento = str_pad(substr($data['num_documento'], -12), 15, ' ', STR_PAD_RIGHT);
            ...
            $segmento_p = $banco.$lote.$registro.$num_registro.$segmento.' '.$cod_mov.$agencia.$convenio.str_pad('', 6, '0', STR_PAD_LEFT).str_pad('', 3, ' ', STR_PAD_RIGHT);
			$segmento_p .= $nosso_numero.$nosso_numero_dv.'11222'.$num_documento.$data_vencimento.$valor_titulo.str_pad('', 6, '0', STR_PAD_LEFT).$especie.$aceite.$data_emissao;
