	...
	class CNAB400 {
		...
		*/
		...
		public function GetRegistrosDetalhe() {
			...
			for ($i = 1; $i < count(file($this->file)) - 1; $i++) :
				...
				if ($IDTransacao == '7') :
					$this->RegistrosDetalhe[$countAux]['TipoInscricaoBeneficiario'] = substr(file($this->file)[$i], 1, 2);
					$this->RegistrosDetalhe[$countAux]['InscricaoBeneficiario'] = substr(file($this->file)[$i], 3, 14);
					$this->RegistrosDetalhe[$countAux]['Agencia'] = substr(file($this->file)[$i], 17, 4);
					$this->RegistrosDetalhe[$countAux]['AgenciaDV'] = substr(file($this->file)[$i], 21, 1);
					$this->RegistrosDetalhe[$countAux]['Conta'] = substr(file($this->file)[$i], 22, 8);
					$this->RegistrosDetalhe[$countAux]['ContaDV'] = substr(file($this->file)[$i], 30, 1);
					$this->RegistrosDetalhe[$countAux]['Convenio'] = substr(file($this->file)[$i], 31, 7);
					$this->RegistrosDetalhe[$countAux]['NossoNumero'] = substr(file($this->file)[$i], 63, 17);
		...
		*/
		...
		public function GetBanco() {
			return substr(file($this->file)[0], 76, 3);
		}
		...
		public function GetConvenio() {
			...
			if ($this->fileType == 'Retorno') :
				$convenio = substr(file($this->file)[0], 149, 7);
			elseif ($this->fileType == 'Remessa') :
				$convenio = substr(file($this->file)[0], 129, 7);	
			endif;
		...
		public function GetAgencia() {
			return substr(file($this->file)[0], 26, 4);	
		}
		
		public function GetAgenciaDV() {
			return substr(file($this->file)[0], 30, 1);
		}
		
		public function GetConta() {
			return substr(file($this->file)[0], 31, 8);
		}
		
		public function GetContaDV() {
			return substr(file($this->file)[0], 39, 1);
		}

		public function GetSacadorAvalista() {
			return substr(file($this->file)[0], 46, 30);	
		}
		...
		public function GetSequencialArquivo() {
			...
			if ($this->fileType == 'Retorno') :
				$sequencial = substr(file($this->file)[0], 107, 7);
			elseif ($this->fileType == 'Remessa') :
				$sequencial = substr(file($this->file)[0], 100, 7);
			endif;
		...
		public function GetDataCriacao() {
			return substr(file($this->file)[0], 94, 6);
		}
