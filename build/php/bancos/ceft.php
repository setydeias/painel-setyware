<?php
	...
	$convenioSRN = '2400000';
	//Arrays auxiliares
	$duplicidades = $pgtoDuplicidade = $arquivos_com_duplicidade = array();
	//Aux vars
			...
			*/
			...
			//Verificação de duplicidades
		 	if ($checarDuplicidade == '1') :
	 			...
	 			$duplicidades[] = verificarDuplicidade($conn, substr($convenio, 1, 6), $data, $dataBase, $dataReferencia, $orig, 'ceft');
	 		endif;
			...
			*/
			for ( $k = 2, $lenK = (count(file($ceft[$i])) - 2); $k < $lenK; $k++ ) :
				...
				if ( $segment == 'T' && substr(file($ceft[$i])[$k], 37, 8) == $convenio ) :
					...
					$customerSigla = $customer->GetSiglaByCodSac(substr($our_number, 7, 3));
				...
				if ($fileName[4] == '0283175' && $segment == ' ' || $segment == '0') :
					...
					$arquivoCHQ = $pathCheque.fmtDatePattern($data, '6').'_104_040450_'.$sigla.'_'.$sequence.'.ret';
					if ($writeInCT == '01') :
									...
									if ( $worksheet->getCellByColumnAndRow(1, $cc)->getValue() == "" ) :
									    ...
									    if ( count($autoat) > 0 || count($agn) > 0 || count($comp) > 0 || count($loterias) > 0 ) :
										    ...
										    if ( count($loterias) > 0 ) :
										   	...
										   	if ( $totalValues > 0 ) :
										   		$getCellInfo = getCellData($objWorksheet, $cc, $t_date, $p_date, $valorTotal, $totalValues, $tarCT, 'processamento', null, null, null, null, 'cef', '1559', $clientsWithPayments, $fileName[2]);
										   	endif;
	...
	*/
	ShowDuplicidades($customer, $duplicidades, $setydeiasConvs, 'ceft');
	/*
	* (6º ETAPA)
	* Remove as duplicidades dos arquivos que as possuem
	*/
	$FileHandler = new FilesHandler();
	$FileHandler->RemoveDuplicatedRecordsCNAB240($arquivos_com_duplicidade, $duplicidades, 'ceft');
	/*
	* (8º ETAPA)
	* Enviar os relatórios de duplicidades para a hospedagem
	*/
	$FileHandler->SendToHost($duplicidades, $dataBase, 'ceft');
?>